...
public class GamePanel extends JPanel implements ActionListener, KeyListener, MouseListener, MouseMotionListener {
    ...
    private void saveGameData() {
        Preferences p = frame.getPrefs();
        p.putInt(EndlessDriveGame.PREF_COINS, playerCoinsTotal);
        p.putInt(EndlessDriveGame.PREF_HIGH_SCORE, highScore);
        p.putInt(EndlessDriveGame.PREF_ACTIVE_SKIN, selectedSkinIdx);
    }

    private boolean isSkinUnlocked(int idx) {
        if (idx == 0) return true; // Default skin is free
        String unlockedList = frame.getPrefs().get(EndlessDriveGame.PREF_UNLOCKED_SKINS, "0");
    ...
    private void unlockSkin(int idx) {
        Preferences p = frame.getPrefs();
        String unlockedList = p.get(EndlessDriveGame.PREF_UNLOCKED_SKINS, "0");
        if (!unlockedList.contains(String.valueOf(idx))) {
            unlockedList += "," + idx;
            p.put(EndlessDriveGame.PREF_UNLOCKED_SKINS, unlockedList);
        }
    }
